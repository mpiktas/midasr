...
test_that("Plain and formula interface give the same results for LSTR", {
  ...
  mfr <- midas_nlpr(y ~ mlsd(y, 1:2, y) + mlsd(x, 0:23, y, nnbeta),
    data = dgp_lstr,
    start = list(
      x = list(r = c(2, 4), lstr = c(1.5, 1, 1, 1)),
      y = c(0.5, 0),
      `(Intercept)` = 1
    ), Ofunction = "optim", method = "Nelder-Mead", control = list(maxit = 100)
  )
  cmap <- c(6:9, 4:5, 1:3)
...
test_that("Plain and formula interface give the same results for MMM", {
  ...
  mfr <- midas_nlpr(y ~ mlsd(y, 1:2, y) + mlsd(x, 0:23, y, nnbeta),
    data = dgp_mmm,
    start = list(
      x = list(r = c(2, 4), mmm = c(1.5, 1)),
      y = c(0.5, 0),
      `(Intercept)` = 1
    ), Ofunction = "optimx", method = "Nelder-Mead"
  )
...
test_that("Rearanging terms works", {
  skip_on_cran()
  mfr1 <- midas_nlpr(y ~ mlsd(y, 1:2, y) + mlsd(x, 0:23, y, nnbeta),
    data = dgp_lstr,
    start = list(
      x = list(r = c(2, 4), lstr = c(1.5, 1, 1, 1)),
      y = c(0.5, 0),
  ...
  mfr2 <- midas_nlpr(y ~ mlsd(x, 0:23, y, nnbeta) + mlsd(y, 1:2, y),
    data = dgp_lstr,
    start = list(
      x = list(r = c(2, 4), lstr = c(1.5, 1, 1, 1)),
      y = c(0.5, 0),
...
test_that("midas_nlpr works with the intercept term only", {
  mfr1 <- midas_nlpr(y ~ mlsd(x, 0:23, y, nnbeta),
    data = dgp_lstr,
    start = list(
      x = list(r = c(2, 4), lstr = c(1.5, 1, 1, 1)),
      `(Intercept)` = 1
    ), Ofunction = "optimx", method = "Nelder-Mead", control = list(maxit = 10)
  )
...
test_that("Updating Ofunction works for nplr", {
  skip_on_cran()
  a <- midas_nlpr(y ~ mlsd(y, 1:2, y) + mlsd(x, 0:23, y, nnbeta),
    data = dgp_lstr,
    start = list(
      x = list(r = c(2, 4), lstr = c(1.5, 1, 1, 1)),
      y = c(0.5, 0),
      `(Intercept)` = 1
    ), Ofunction = "optim", method = "Nelder-Mead", control = list(maxit = 5000)
  )
...
test_that("updating data and starting values works", {
  ...
  c <- midas_nlpr(y ~ mlsd(y, 1:2, y) + mlsd(x, 0:23, y, nnbeta),
    data = spd,
    start = list(
      x = list(r = c(2, 4), lstr = c(1.5, 1, 1, 1)),
      y = c(0.5, 0),
  ...
  b <- update(a, data = spd, start = list(
    x = list(r = c(2, 4), lstr = c(1.5, 1, 1, 1)),
    y = c(0.5, 0),
    `(Intercept)` = 1
  ), Ofunction = "optim", method = "Nelder-Mead", control = list(maxit = 5000))
...
test_that("LSTR standard errors work", {
  ...
  mfr <- midas_nlpr(y ~ mlsd(y, 1:2, y) + mlsd(x, 0:23, y, nnbeta),
    ...
    start = list(
      ...
      y = c(0.5, 0),
      `(Intercept)` = 1
    ), Ofunction = "optimx", method = "Nelder-Mead", control = list(maxit = 5000)
  )
